...
@Component({
  selector: 'app-sadmin-greeting',
  templateUrl: './sadmin-greeting.component.html',
  styleUrls: ['./sadmin-greeting.component.css']
})
export class SAdminGreetingComponent implements OnInit {
  page: number = 1;
  pageBar: number[] = [];
  maxPage!: number;
  ...
  async queryData() {
    ...
    try {
      await new Promise((resolve, reject) => {
        this.http.get<ApiResponse>(environment.paths.getGreeting + '/?page='+this.page+'&pageSize=50' + (this.filter ? '&filter=' + this.filter : '/'), { withCredentials: true }).subscribe(res => {
    ...
    try {
      ...
      await new Promise<ApiResponse>((resolve, reject) => {
        this.http.post<ApiResponse>(environment.paths.createGreeting, {
          organizationId: this.addMessageOrgInput ? this.addMessageOrgInput : null,
      ...
      await new Promise<ApiResponse>((resolve, reject) => {
        ...
        this.http.put<ApiResponse>(environment.paths.updateGreeting, {
          ...
          if (res.statusCode === 200) {
            ...
            this.render[this.currentIndex].emotion = this.addEmotionInput || '';
            this.memory[this.page] = this.render
            this.clearInput()
          }
          resolve(res)
  ...
  clearInput() {
    ...
    this.addMessageOrgInput = undefined;
